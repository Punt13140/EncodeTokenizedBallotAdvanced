      ...
      <div className="flex items-center flex-col flex-grow pt-10">
        <div className="px-5">
          <h1 className="text-center">
            <span className="block text-4xl font-bold">Tokenized Ballot - Team 3</span>
          </h1>
          <PageBody></PageBody>
        </div>
      </div>
  ...
  if (address)
    return (
      <div>
        ...
        <WalletAction address={address as `0x${string}`}></WalletAction>
...
function RequestTokens(params: { address: string; signature: string }) {
  const [data, setData] = useState<{ result: boolean }>();
  const [isLoading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const body = { address: params.address, signature: params.signature };
  ...
  const handleButtonClick = async () => {
    ...
    try {
      const response = await fetch("http://localhost:3001/request-tokens", {
        method: "POST",
  ...
  if (!data)
    return (
      <button className="btn btn-active btn-neutral" onClick={handleButtonClick} disabled={isLoading}>
        Request tokens
      </button>
    );
...
function WalletAction(params: { address: `0x${string}` }) {
  ...
  return (
    <div className="card w-96 bg-primary text-primary-content mt-4">
      <div className="card-body">
        ...
        {isSuccess && data && <RequestTokens address={params.address} signature={data}></RequestTokens>}
